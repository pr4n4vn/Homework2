...
#2
for(i in 1:20) {
    if(my_df$post_bp[i] <= 120) {
      my_df$status[i] = "Optimal" 
    } else if(my_df$post_bp[i] > 120 & my_df$post_bp[i] <= 130) {
      my_df$status[i] = "Borderline"
    } else if(my_df$post_bp[i]> 130) {
      my_df$status[i] = "High"
    } else {
      my_df$status[i] = "error"
    }
}
my_df
...
#3
for(i in 1:20) {
    if(my_df2$post_bp2[i] <= 120) {
      my_df2$status[i] = "Optimal" 
    } else if(my_df2$post_bp2[i] > 120 & my_df2$post_bp2[i] <= 130) {
      my_df2$status[i] = "Borderline"
    } else if(my_df2$post_bp2[i]> 130) {
      my_df2$status[i] = "High"
    } else {
      my_df2$status[i] = "error"
    }
}
...
```{r}
#1
summarizer <- function(dflist, summary = mean, ) {
  if(!is.vector())
}
```
